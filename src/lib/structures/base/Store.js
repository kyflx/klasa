...
class Store extends Collection {
	...
	load(directory, file) {
		...
		try {
			...
			if (this.client.listenerCount('wtf')) this.client.emit('wtf', { prefix: `store ${this.name}`, message: error, suffix: file.slice(0, -1) });
			else this.client.logger.fatal(`Failed to load file '${loc}'. Error:\n${error.stack || error}`);
		}
